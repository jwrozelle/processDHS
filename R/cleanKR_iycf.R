#' Clean and Compute Infant and Child Feeding Indicators
#'
...
#' @return A data frame with new variables indicating the feeding status and dietary practices of children.
#' 
#' @import dplyr
#' @import haven
#' @import naniar
...
cleanKR_iycf <- function(KRdata) {
  ...
  KRdata <- KRdata %>%
    mutate(nt_ageapp_bf =
             case_when(
               nt_ebf==1 | nt_bf_status==5 & data.table::inrange(age,6,23)  ~ 1,
               TRUE ~ 0 )) %>%
  ...
  KRdata <- KRdata %>%
    mutate(nt_food_bf =
             case_when(
               solids==1 & data.table::inrange(age,6,8)  ~ 1,
               solids==0 & data.table::inrange(age,6,8) ~ 0 )) %>%
  ...
  KRdata <- KRdata %>%
    mutate(nt_bf_cont_1yr =
             case_when(
               m4==95 & data.table::inrange(age,12,15)  ~ 1,
               m4!=95 & data.table::inrange(age,12,15) ~ 0 )) %>%
  ...
  KRdata <- KRdata %>%
    mutate(nt_bf_cont_2yr =
             case_when(
               m4==95 & data.table::inrange(age,20,23)  ~ 1,
               m4!=95 & data.table::inrange(age,20,23) ~ 0 )) %>%
  ...
  KRdata <- KRdata %>%
    ...
    mutate(nt_fed_milk  = 
             case_when(totmilkf>=2 | m4==95 & data.table::inrange(age,6,23) ~ 1 , 
                       totmilkf <2 | m4!=95 & data.table::inrange(age,6,23) ~ 0)) %>%
  ...
  KRdata <- KRdata %>%
    ...
    mutate(nt_mdd  = case_when(data.table::inrange(age,6,23) & foodsum<5 ~ 0 , data.table::inrange(age,6,23) & foodsum>=5~ 1)) %>% 
  ...
  KRdata <- KRdata %>%
    ...
    mutate(nt_mmf  = 
             if_else(data.table::inrange(age,6,23) & (m4==95 & data.table::inrange(m39,2,7) & data.table::inrange(age,6,8)) | (m4==95 & data.table::inrange(m39,3,7) & data.table::inrange(age,9,23)) |
                       (m4!=95 & feedings>=4 & data.table::inrange(age,6,23)), 1, 0 )) 
  ...
  KRdata[["nt_ch_micro_vaf"]] <- ifelse(KRdata[["age"]]<6 | KRdata[["age"]]>23  , NA, KRdata[["nt_ch_micro_vaf"]])  
  KRdata <- KRdata %>%
  ...
  KRdata[["nt_ch_micro_irf"]] <- ifelse(KRdata[["age"]]<6 | KRdata[["age"]]>23  , NA, KRdata[["nt_ch_micro_irf"]])  
  KRdata <- KRdata %>%
